...
class Simple_login
{
    ...
    */
    ...
    public function loginuser($username, $password)
    {
        ...
        //cek username dan password
        $query = $this->CI->db->get_where('customer', array('username' => $username, 'password' => $password));

        if ($query->num_rows() == 1) {
            //ambil data user berdasar username
            $row  = $this->CI->db->query('SELECT id_customer FROM customer where username = "' . $username . '"');
            $admin     = $row->row();
            $id   = $admin->id_customer;

            //set session user
            $this->CI->session->set_userdata('username', $username);
            $this->CI->session->set_userdata('id_customer', uniqid(rand()));
            $this->CI->session->set_userdata('id', $id);

            //redirect ke halaman dashboard
            redirect(site_url('maxtune'));
        } else {
